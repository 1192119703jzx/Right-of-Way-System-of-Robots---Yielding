...
class RoadFollower:
    ...
    def image_cb(self, msg):
        ...
        cv2.imshow("Mask Left rafael", mask_left)
        h, w = mask_left.shape
        ...
        cv2.imshow("Mask Right rafael", mask_right)
        ...
        cv2.imshow("window rafael", cv_image)
        cv2.waitKey(3)
    ...
    def follow_road(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.pin_id == None:
                ...
                self.pin_id = pin_id
            print(f"rafael {self.pin_id}")
            try:
                A_tf = self.tf_buffer.lookup_transform("rafael/base_link", f"pin_{self.pin_id}", rospy.Time())
                dist = math.sqrt(A_tf.transform.translation.x**2 + A_tf.transform.translation.y**2)
                print(f"rafael distance to fid = {dist}")
                if dist < 0.7:
                    print("got you")
                    self.stage = "cross"
            ...
            if self.has_left and self.has_right:
                print("rafael Drive on the right side of the road")
