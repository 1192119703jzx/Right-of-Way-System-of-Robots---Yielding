...
class RoadFollower:
    def __init__(self):
        ...
        self.pin_id = None
        self.ranges = []
        self.scan_sub = rospy.Subscriber('/rafael/scan', LaserScan, self.scan_cb)
    ...
    def follow_road(self):
        ...
        while not rospy.is_shutdown():
            print(f"rafael {self.has_left},{self.has_right}, moving1")
            while not self.ranges:
                rospy.sleep(0.1)
            left = self.cleanMin(self.ranges[19:48])
            print(left)
            if left == float('inf'):
                continue
            elif left > 0.35:
    ...
    def follow_continue(self):
        ...
        print("3")
        while not rospy.is_shutdown():
            print(f"rafael {self.has_left},{self.has_right}, leaving")
            if self.has_left and self.has_right:
                curr_error = (self.w / 2 - (self.x + (self.x2 - self.x) / 2)) / 100
                sum_error += curr_error * self.PID_FOR_IMAGE[3]
                PID_cal = self.PID_FOR_IMAGE[0] * curr_error #+ self.PID_FOR_IMAGE[2] * (curr_error - prev_error) / self.PID_FOR_IMAGE[3] + self.PID_FOR_IMAGE[1] * sum_error 
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            if self.state == "Passing":
                ...
                print("Rafael, You shall not pass")
